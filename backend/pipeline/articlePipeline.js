...
// Threshold constants
const PROCESS_GATE_THRESHOLD_VALUE = PROCESS_GATE_THRESHOLD; // Gate for expensive processing (content fetch + deep LLM)
const FEED_RANK_THRESHOLD = 40; // Minimum score to appear in feed (ranking, not gating)
...
class ArticlePipeline {
  ...
  async processArticle(article, userHoldings = [], userProfile = "balanced") {
    ...
    if (!impactGuess.should_fetch) {
      console.log(`[Pipeline] Article below process gate threshold (${impactGuess.likely_impact} < ${PROCESS_GATE_THRESHOLD_VALUE}): ${article.url}`);
  ...
  async processBatch(articles, userHoldings = [], userProfile = "balanced", options = {}) {
    ...
    // Prerequisites: status = "title_filtered", should_fetch_full = 1, likely_impact IS NULL
    console.log(`\n[Pipeline Batch] ========== STAGE 1.5: Lightweight Impact Guess ==========`);
    console.log(`[Pipeline Batch] Checking articles for Stage 1.5 prerequisites...`);
    
    const articlesForStage1_5 = [];
    const skipReasons1_5 = {
      notFound: 0,
      discarded: 0,
      wrongStatus: 0,
      shouldNotFetch: 0,
    ...
    for (const article of articles) {
      const existing = db.prepare(`
        SELECT status, should_fetch_full, likely_impact
        FROM articles WHERE url = ?
      `).get(article.url);
      
      // Check prerequisites for Stage 1.5
      if (!existing) {
        skipReasons1_5.notFound++;
        continue;
      }
      if (existing.status === "discarded") {
        skipReasons1_5.discarded++;
        continue;
      }
      if (existing.status !== "title_filtered") {
        skipReasons1_5.wrongStatus++;
        continue;
      }
      if (existing.should_fetch_full !== 1) {
        skipReasons1_5.shouldNotFetch++;
        continue;
      }
      if (existing.likely_impact !== null && existing.likely_impact !== undefined) {
        // Already guessed, check if should proceed
        skipReasons1_5.alreadyGuessed++;
        if (existing.likely_impact >= PROCESS_GATE_THRESHOLD_VALUE) {
          ...
          results.push({ status: "low_priority", stage: 1.5, likely_impact: existing.likely_impact });
          continue;
        }
      }
      
      articlesForStage1_5.push(article);
    }
    
    // Process Stage 1.5 (lightweight impact guess)
    if (articlesForStage1_5.length > 0) {
      console.log(`[Pipeline Batch] Processing ${articlesForStage1_5.length} articles through Stage 1.5...`);
      for (const article of articlesForStage1_5) {
        const impactGuess = decideImpactLite(article);
        if (!impactGuess.should_fetch) {
          db.prepare(`UPDATE articles SET status = 'low_priority' WHERE url = ?`).run(article.url);
      ...
      console.log(`[Pipeline Batch] Stage 1.5 Complete: Processed ${articlesForStage1_5.length} articles`);
    } else {
      console.log(`[Pipeline Batch] Stage 1.5: No articles need processing, skipping`);
    }

    // ========== STAGE 2: Content Fetch ==========
    // Prerequisites: status = "title_filtered", should_fetch_full = 1, likely_impact >= PROCESS_GATE_THRESHOLD, clean_text IS NULL
    console.log(`\n[Pipeline Batch] ========== STAGE 2: Content Fetch ==========`);
    console.log(`[Pipeline Batch] Checking articles for Stage 2 prerequisites (likely_impact >= ${PROCESS_GATE_THRESHOLD_VALUE})...`);
    ...
    for (const article of articles) {
      const existing = db.prepare(`
        SELECT status, should_fetch_full, clean_text, content_length, fetch_attempts, likely_impact
        FROM articles WHERE url = ?
      `).get(article.url);
      ...
      // NEW: Check likely_impact threshold (process gate)
      if (existing.likely_impact === null || existing.likely_impact === undefined) {
        // Likely impact not set yet, skip (should have been set in Stage 1.5)
    ...
    for (const article of articles) {
      ...
      // Stage 4 processes articles for personalization (score adjustment)
      // Use a low threshold here - FEED_RANK_THRESHOLD is applied at feed query time, not here
      const STAGE4_MIN_IMPACT = 20; // Minimum impact to proceed to personalization
      if (existing.impact_score < STAGE4_MIN_IMPACT) {
        stage4SkipReasons.lowImpact++;
        continue; // Skip very low impact
      }
      // NOTE: matched_holdings is NOT required - articles without holdings can still be personalized
      // Macro/market news without holdings matches should still go through Stage 4
      ...
      // NOTE: matchedHoldings.length === 0 is NOT a skip condition
      // Articles without holdings matches can still be personalized (macro/market news)
      // Stage 4 will handle this with lower base scores
