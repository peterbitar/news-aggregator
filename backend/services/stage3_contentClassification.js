...
async function processContentClassification(article, userHoldings = []) {
  ...
  // GUARDRAIL 5B: Check first 500 chars for ticker/issuer mention
  const first500 = (articleRow.clean_text || "").substring(0, 500).toUpperCase();
  const searchedBy = articleRow.searched_by || "";
  const hasMention = checkMentionInFirst500Chars(first500, searchedBy, userHoldings);
  
  if (!hasMention) {
    console.log(`[Stage 3] Skipping ${article.url} - no ticker/issuer mention in first 500 chars`);
    // Mark as low impact and discard
    db.prepare(`
      UPDATE articles SET
        impact_score = 0,
        status = 'discarded',
      ...
      WHERE url = ?
    `).run(article.url);
    return { status: "discarded", reason: "No mention of holding in first 500 chars" };
  }
...
Article Content:
${articleRow.clean_text.substring(0, 8000)}${articleRow.clean_text.length > 8000 ? "..." : ""}

User Holdings: ${holdingsList.length > 0 ? holdingsList.map(h => `${h.ticker} (${h.label})`).join(", ") : "None"}${searchContext}
...
Return ONLY valid JSON, no markdown formatting.`;
    ...
    const tickersArray = JSON.parse(matched_tickers);
    holdingsList.forEach((holding) => {
      if (tickersArray.includes(holding.ticker.toUpperCase())) {
        matchedHoldings.push(holding.ticker);
      }
    });
    const matched_holdings = JSON.stringify(matchedHoldings);
...
async function processContentClassificationBatch(articles, userHoldings = []) {
  ...
  try {
    ...
    // GUARDRAIL 5B: Pre-drop articles that don't mention holdings/tickers in first 500 chars
    const preFiltered = [];
    const preDropped = [];
    ...
    for (let i = 0; i < articlesToProcess.length; i++) {
      ...
      // GUARDRAIL 5B: Check first 500 chars for ticker/issuer mention
      const first500 = (data.clean_text || "").substring(0, 500).toUpperCase();
      const hasMention = checkMentionInFirst500Chars(first500, data.searched_by, userHoldings);
      
      if (hasMention) {
        preFiltered.push({ article, data, index: i });
      } else {
        // Mark as dropped - no mention in first 500 chars
        preDropped.push({ article, data, reason: "No mention of holding in first 500 chars" });
      }
    }
    
    // Save pre-dropped articles
    if (preDropped.length > 0) {
      ...
      dropTransaction(preDropped);
      console.log(`[Stage 3 Batch] Pre-dropped ${preDropped.length} articles without LLM call`);
    }
    
    if (preFiltered.length === 0) {
      return articles.map(() => ({ skipped: true, preDropped: true }));
    }
...
User Holdings: ${holdingsList.length > 0 ? holdingsList.map(h => `${h.ticker} (${h.label})`).join(", ") : "None"}
...
Return a JSON object with analysis for each article URL.`;
    ...
    console.log(`[Stage 3 Batch] Pass 2: Processed ${articlesForPass2.length} articles in 1 LLM call (${preDropped.length} pre-dropped, ${pass1Dropped.length} Pass 1 dropped)`);
    return results;
