...
function getFeedArticles(options = {}) {
  ...
  // Holdings are used for PRIORITIZATION only, not filtering
  // All articles meeting the score threshold are included, but holdings-related ones are ranked higher
  if (holdings && holdings.length > 0) {
    const tickers = holdings.map(h => (typeof h === 'string' ? h : h.ticker).toUpperCase());
    console.log(`[getFeedArticles] Using ${tickers.length} holdings for prioritization (not filtering): ${tickers.join(', ')}`);
    
    // Build holdings boost expression for ORDER BY (not WHERE)
    // This ensures all articles are included, but holdings-related ones rank higher
    const holdingsBoostParts = tickers.map(() => 
      `CASE WHEN searched_by LIKE '%' || ? || '%' THEN 10 ELSE 0 END`
    );
    const holdingsBoost = holdingsBoostParts.join(' + ');
    
    // Add ORDER BY with holdings boost
    sql += ` ORDER BY (COALESCE(final_rank_score, profile_adjusted_score) + (${holdingsBoost})) DESC, profile_adjusted_score DESC, published_at DESC LIMIT ?`;
    
    // Add ticker parameters for the holdings boost CASE statements, then limit
    params.push(...tickers, limit);
  } else {
    console.log(`[getFeedArticles] No holdings provided - showing all articles ranked by score only`);
    // Standard ORDER BY without holdings boost
...
function getRankedForFeed(options = {}) {
  ...
  const rows = db.prepare(`
    ...
    WHERE status = 'ranked'
      AND status != 'discarded'
      AND (shown_to_user = 1 OR final_rank_score >= ?)
      AND final_rank_score IS NOT NULL
    ORDER BY final_rank_score DESC, published_at DESC
    LIMIT ?
  `).all(FEED_RANK_THRESHOLD, limit);
  return rows;
}

/**
 * Update final interpretation fields (Signal DTO fields) for an article
 * Called after pipeline stage 5 completes
 * @param {string} urlOrId - Article URL or ID
 * @param {Object} finalFields - Signal fields to update
 */
function updateFinalInterpretation(urlOrId, finalFields) {
  const db = getDatabase();
  
  const updates = [];
  const params = [];
  
  if (finalFields.verdict !== undefined) {
    updates.push("verdict = ?");
    params.push(finalFields.verdict);
  }
  if (finalFields.why_json !== undefined) {
    updates.push("why_json = ?");
    params.push(typeof finalFields.why_json === 'string' 
      ? finalFields.why_json 
      : JSON.stringify(finalFields.why_json));
  }
  if (finalFields.action !== undefined) {
    updates.push("action = ?");
    params.push(finalFields.action);
  }
  if (finalFields.horizon !== undefined) {
    updates.push("horizon = ?");
    params.push(finalFields.horizon);
  }
  if (finalFields.opportunity_type !== undefined) {
    updates.push("opportunity_type = ?");
    params.push(finalFields.opportunity_type);
  }
  if (finalFields.opportunity_note !== undefined) {
    updates.push("opportunity_note = ?");
    params.push(finalFields.opportunity_note);
  }
  if (finalFields.confidence !== undefined) {
    updates.push("confidence = ?");
    params.push(finalFields.confidence);
  }
  if (finalFields.importance_score !== undefined) {
    updates.push("importance_score = ?");
    params.push(finalFields.importance_score);
  }
  ...
  const sql = `UPDATE articles SET ${updates.join(', ')} WHERE url = ?`;
  
  try {
    db.prepare(sql).run(...params);
  } catch (error) {
    console.error(`[updateFinalInterpretation] Error updating ${urlOrId}:`, error.message);
    throw error;
  }
}
